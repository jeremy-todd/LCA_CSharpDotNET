...
namespace ToDoList
{
    class ItemRepository
    {
        ...
        public static void UpdateItem(int todoID, string desc, string dueDate, string status, string priority)
        {
            ToDoItem UpdatedToDoItem = todoList.ToDoList.Where(x => x.ID == todoID).FirstOrDefault();
        ...
        public static bool ToDoIDVerify(int todoID)
        {
            bool verifyID = todoList.ToDoList.Any(x => x.ID == todoID);
            return verifyID;
        }
    }
}